...
[NOTE]
====
Note that this automation will run automatically when the issue
for the previous month's mass rebuild is closed. So don't close
the previous months issue until the next months rebuild is complete.
===
